...
fun ConcertDetailScreen(
    ...
    onBack: () -> Unit = {},
    onEdit: (ConcertEntity) -> Unit = {},
    onDelete: (Long) -> Unit = {}
) {
    // 使用collectAsState收集单个演出数据
    ...
    Column(
        ...
        TopAppBar(
            ...
            actions = {
                IconButton(onClick = { concert?.let { onEdit(it) } }) {
                    Icon(Icons.Filled.Edit, contentDescription = "编辑")
                }
                IconButton(onClick = { concert?.id?.let { id -> onDelete(id) } }) {
...
@Composable
fun ConcertDetailContent(concert: ConcertEntity) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
